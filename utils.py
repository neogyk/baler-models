...
def gaussian_latent_ppf(mean, stdd, prior_prec, post_prec):
    def ppf(cf):
        ...
        return np.searchsorted(std_gaussian_buckets(prior_prec), x, "right") - 1

    return ppf


def beta_latent_cdf(a_prior, b_prior, a_post, b_post, prior_prec, post_prec):
    def cdf(idx):
...
def beta_latent_ppf(a_prior, b_prior, a_post, b_post, prior_prec, post_prec):
    def ppf(cf):
...
def vae_append(
    latent_shape, gen_net, rec_net, obs_append, prior_prec=8, latent_prec=12
):
    """
    ...
    def post_pop(data):
        ...
        cdfs = [
            gaussian_latent_cdf(m, s, prior_prec, latent_prec)
            for m, s in zip(post_mean, post_stdd)
        ]
        ppfs = [
            gaussian_latent_ppf(m, s, prior_prec, latent_prec)
            for m, s in zip(post_mean, post_stdd)
        ]
...
def vae_pop(latent_shape, gen_net, rec_net, obs_pop, prior_prec=8, latent_prec=12):
    """
    Assume that the vae uses an isotropic Gaussian for its prior and diagonal
    Gaussian for its posterior.
    """
...
def beta_binomial_log_pdf(k, n, a, b):
    a_plus_b = a + b
    numer = gammaln(n + 1) + gammaln(k + a) + gammaln(n - k + b) + gammaln(a_plus_b)
    denom = (
        gammaln(k + 1)
        + gammaln(n - k + 1)
        + gammaln(n + a_plus_b)
